...
class NewTakeSolPage extends StatefulWidget {
  const NewTakeSolPage({super.key});

  @override
  NewTakeSolPageState createState() => NewTakeSolPageState();
}

class NewTakeSolPageState extends State<NewTakeSolPage> {
  ...
  Future<void> _fetchUserData() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await _fetchNewSolution(); // 솔루션 데이터 가져오기
      } else {
        setState(() {
  ...
  Future<void> _fetchNewSolution() async {
    if (direction == null || userId == null) return;

    try {
      final response = await http.get(
        Uri.parse('https://4729-192-203-145-70.ngrok-free.app/new_takesol?_id=$userId&direction=$direction'),
      );
